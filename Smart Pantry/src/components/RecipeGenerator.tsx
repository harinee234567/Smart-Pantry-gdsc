    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-semibold text-pantry-brown">Recipe Generator</DialogTitle>
            <DialogDescription>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {category.items.map((ingredient) => (
                        <div key={ingredient} className="flex items-center space-x-2">
                          <Checkbox 
                            id={`${category.name}-${ingredient}`} 
                            checked={selectedIngredients.includes(ingredient)}
                            onCheckedChange={() => toggleIngredient(ingredient)}
                          />
                          <Label htmlFor={`${category.name}-${ingredient}`} className="text-sm cursor-pointer">
                            {ingredient}
                          </Label>
                        </div>
                      ))}
